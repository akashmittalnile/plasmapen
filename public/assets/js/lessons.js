    ...
    if (div_type == "video") {
                    ...
                    <div class="question">
                        <div class="plas-edit-questionnaire-box">
                            ...
                            <div class="plas-questionnaire-ans mb-0" style="padding: 5px; width: 15%;">
                                <div class="plas-questionnaire-text">
                                    <input type="number" class="form-control" placeholder="Enter marks" name="question[${countForm}][${questionCounter}][marks]" value="" required style="padding: 6px;">
                                </div>
                            </div>
                        </div>
        ...
        htmlForm = `<div class="add-course-form-item section-item">
                <div class="add-course-heading">
                    ...
                    <div class="add-course-action">
                        <a href="javascript:void(0)" class="btndelete dlt-div" data-type="assignment"> Delete Section</a>
                    </div>
                </div>
                <input type="hidden" name="type[${countForm}]" id="type${countForm}" value="assignment" />
                <input type="hidden" name="queue[${countForm}]" id="type${countForm}" value="${countForm}" />
                <div class="add-course-content-section add-course-form">
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control" name="assignment_title[${countForm}][${assignmentCounter}][text]" value="" placeholder="Enter title" id="assignment-title-${countForm}-${assignmentCounter}" required>
                                    </div>
                                </div>
                                <div class="col-md-12">
                                    <div class="form-group">
                                        <h4>Assignment Decription</h4>
                                        <textarea type="text" class="form-control" required id="assignment-description-${countForm}-${assignmentCounter}" name="assignment_description[${countForm}][${assignmentCounter}][description]" placeholder="Enter decription"></textarea>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <a class="add-answer assignment-add-option mt-2" href="javascript:void(0)" id="assignment-addOption-${countForm}-${assignmentCounter}">Add Option</a>
                </div>
            </div>`;
        countForm += 1;
    } else if (div_type == "survey") {
        let oplength = $('.survey-options .plas-answer-box .hidden'+countForm+questionCounter).length;
        htmlForm = `<div class="add-course-form-item section-item">
                <div class="add-course-heading">
                    <div class="plas-course-text">
                        <h3>Survey</h3>
                    ...
                    <div class="add-course-action">
                        <a href="javascript:void(0)" class="btnAddQuestion survey-add-question-create" id="survey-addQuestion-${countForm}"> Add Question</a>
                        <a href="javascript:void(0)" class="btndelete dlt-div" data-type="survey"> Delete Section</a>
                    </div>
                </div>

                <input type="hidden" name="type[${countForm}]" id="type${countForm}" value="survey" />
                <input type="hidden" name="queue[${countForm}]" id="type${countForm}" value="${countForm}" />

                <div class="add-course-content-section survey-questions-${countForm}">
                    ...
                    <div class="survey-question">
                        <div class="plas-edit-questionnaire-box">
                            ...
                            <div class="plas-questionnaire-content">
                                <input type="text" class="form-control" placeholder="Enter question" name="survey_question[${countForm}][${questionCounter}][text]" id="survey-question-${countForm}-${questionCounter}" required>
                            </div>
                        </div>
                        <div class="lp-answer-option-list">
                            <div class="survey-options">
                                <div class="plas-answer-box">
                                    ...
                                    <div class="plas-questionnaire-ans">
                                        ...
                                        <div class="plas-questionnaire-text">
                                            <input type="text" class="form-control" placeholder="Type Here..." name="survey_question[${countForm}][${questionCounter}][options][]" value="" required>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <a class="add-answer survey-add-option mt-2" href="javascript:void(0)" id="survey-addOption-${countForm}-${questionCounter}">Add Option</a>
                        </div>
                    </div>
...
const previewImage = (event) => {
    ...
    $(`.assignment-box-logo-${count}-${id}`).attr("href", URL.createObjectURL(event.target.files[0]));
    $(`.assignment-box-logo-${count}-${id}`).removeClass('d-none');
}
...
$(document).on('click', '.add-question-create', function () {
    ...
    var html = `<div class="question">
            <div class="plas-edit-questionnaire-box">
                ...
                <div class="plas-questionnaire-ans mb-0" style="padding: 5px; width: 15%;">
                    <div class="plas-questionnaire-text">
                        <input type="number" class="form-control" placeholder="Enter marks" name="question[${id}][${questionCounter}][marks]" value="" required style="padding: 6px;">
                    </div>
                </div>
            </div>
            <div class="lp-answer-option-list">
...
$(document).on('click', '.survey-add-option', function () {
    let id = ($(this).attr('id').split('-'));
    let oplength = $('.survey-options .plas-answer-box .hidden'+id[2]+questionCounter).length;
    var op_html = `<div class="survey-options">
            <div class="plas-answer-box">
                <input type="hidden" class="hidden${id[2]}${questionCounter}" value="0">
                <div class="plas-questionnaire-ans">
                    <div class="plas-ans-label">
                        <div class="a-badge">A</div>
                    </div>
                    <div class="plas-questionnaire-text">
                        <input type="text" class="form-control" placeholder="Type Here..." name="survey_question[${id[2]}][${id[3] ?? questionCounter}][options][]" value="" required>
                    </div>
                    <div class="plas-answer-action-item">
                        <div class="plas-btn-info">
                            <button class="remove-btn survey-remove-option">Remove</button>
                        </div>
                    </div>
                </div>
            </div>
        </div>`;
    $(this).siblings('.survey-options').append(op_html);
});

$(document).on('click', '.survey-remove-option', function () {
    $(this).closest('.survey-options').remove();
});

$(document).on('click', '.survey-add-question-create', function () {
    let id = ($(this).attr('id').split('-'))[2];
    questionCounter++;
    let oplength = $('.survey-options .plas-answer-box .hidden'+id+questionCounter).length;
    var html = `<div class="survey-question">
            <div class="plas-edit-questionnaire-box">
                <div class="plas-label">
                    <div class="plas-badge">Q</div>
                </div>
                <div class="plas-questionnaire-content">
                    <input type="text" class="form-control" placeholder="Enter question" name="survey_question[${id}][${questionCounter}][text]" id="survey-question-${id}-${questionCounter}" required>
                </div>
            </div>
            <div class="lp-answer-option-list">
                <div class="survey-options">
                    <div class="plas-answer-box">
                        ...
                        <div class="plas-questionnaire-ans">
                            ...
                            <div class="plas-questionnaire-text">
                                <input type="text" class="form-control" placeholder="Type Here..." name="survey_question[${id}][${questionCounter}][options][]" value="" required>
                            </div>
                        </div>
                    </div>
                </div>
                <a class="add-answer survey-add-option mt-2" href="javascript:void(0)" id="survey-addOption-${id}-${questionCounter}">Add Option</a>
                <button class="remove-btn survey-remove-question" style="font-weight: 600; font-size: 14px; text-transform: uppercase;">Remove Question</button>
            </div>
        </div>`;
    $('.survey-questions-'+id).append(html);
});

$(document).on('click', '.survey-remove-question', function () {
    $(this).closest('.survey-question').remove();
});

$(document).on('click', '.assignment-add-option', function () {
    let id = ($(this).attr('id').split('-'));
    assignmentCounter++;
    var op_html = `<div class="assignment-section">
        <div class="plas-questionnaire-ans d-block mt-3">
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-group">
                            <h4>Assignment Title </h4>
                            <input type="text" class="form-control" name="assignment_title[${id[2]}][${assignmentCounter}][text]" value="" placeholder="Enter title" id="assignment-title-${countForm}-${assignmentCounter}" required>
                        </div>
                    </div>
                    <div class="col-md-12">
                        <div class="form-group">
                            <h4>Assignment Decription</h4>
                            <textarea type="text" class="form-control" required id="assignment-description-${countForm}-${assignmentCounter}" name="assignment_description[${id[2]}][${assignmentCounter}][description]" placeholder="Enter decription"></textarea>
                        </div>
                    </div>
                </div>
                <button class="remove-btn assignment-remove-option">Remove</button>
            </div>
        </div>`;
    $(this).siblings('.assignment-section').append(op_html);
});

$(document).on('click', '.assignment-remove-option', function () {
